...
class AdminAdjustLockLevel extends RowAction
{
    public $name = '自动降级';
    ...
    public function handle(Member $model, Request $request)
    {
        try{
            ...
            $result = ChangeOrder::adjustMemberStatus($model,$tostatus,8);
            if($result){
    ...
    {
        $this->radio('to_status', '用户降级状态')
            ->options(Member::LOCK_SHOP_LEVEL_MAP)
            ->rules('required');
    }
