...
class AdminAdjustMemberLevelAction extends RowAction
{
    public $name = '变更等级';
    ...
    public function handle(Model $model, Request $request)
    {
        try{
            ...
            if(!in_array($toLevel, array_keys(Level::getName()))) {
                throw new BizException('非法的等级');
            }
            $result = ChangeOrder::adjustMemberLevel($model,$toLevel);
            if($result){
